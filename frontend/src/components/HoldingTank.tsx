...
const HoldingTank = () => {
    ...
    if (!pendingUsers || pendingUsers.length === 0) return (
        <div className="bg-white dark:bg-slate-800 rounded-lg p-8 border border-gray-200 dark:border-slate-700 text-center shadow-lg dark:shadow-none">
            <div className="w-12 h-12 bg-gray-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-3">
                <CheckCircle className="text-green-500" size={24} />
            </div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Holding Tank Empty</h3>
            <p className="text-gray-500 dark:text-slate-400 mt-1">All your sponsored members have been placed.</p>
        </div>
    );

    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden shadow-lg dark:shadow-none">
            <div className="p-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-700/20">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <User className="text-teal-600 dark:text-teal-400" size={20} />
                    Holding Tank <span className="text-sm bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 px-2 py-0.5 rounded-full">{pendingUsers.length}</span>
                </h2>
                {message && (
            ...
            <div className="p-4 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                    <p className="text-sm text-gray-500 dark:text-slate-400 mb-2">Select a member to place:</p>
                    {pendingUsers.map((user: any) => (
                        <div
                            ...
                            className={`p-3 rounded-lg border cursor-pointer transition-all flex items-center justify-between ${selectedUser?._id === user._id
                                ...
                                : 'bg-white dark:bg-slate-700/30 border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'
                                }`}
                        >
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-slate-600 flex items-center justify-center font-bold text-xs text-gray-700 dark:text-slate-300">
                ...
                {/* Placement Form */}
                <div className={`bg-gray-50 dark:bg-slate-900/50 rounded-lg p-5 border border-gray-200 dark:border-slate-700 flex flex-col justify-center ${!selectedUser ? 'opacity-50 pointer-events-none' : ''}`}>
                    <h3 className="text-gray-900 dark:text-white font-medium mb-4 flex items-center gap-2">
                        Place <span className="text-teal-600 dark:text-teal-400 font-bold">{selectedUser?.username || '...'}</span>
                    </h3>
